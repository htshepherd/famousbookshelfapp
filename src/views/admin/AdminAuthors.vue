  ...
  <div class="admin-page">
    <div class="page-header">
      <div class="header-text">
        <h1>Authors</h1>
        <p>Manage author profiles and their information</p>
      </div>
      <el-button type="primary" @click="openDrawer()" id="add-author-btn" class="add-btn">
    ...
    <div class="toolbar">
      <div class="search-bar">
        ...
        <input v-model.trim="searchQuery" @keyup.enter="handleSearch" type="text" placeholder="Search authors by name..." />
      </div>
      <div class="toolbar-actions">
        <span>Showing {{ tableData.length }} of {{ pagination.total }} authors</span>
      </div>
    </div>

    <div class="people-list" v-loading="loading">
      <div v-for="row in tableData" :key="row.authorId" class="people-card">
        <div class="avatar-wrap">
          <img v-if="row.avatarUrl" :src="row.avatarUrl" />
        ...
        <div class="people-info">
          ...
          <div class="people-sub">
            ...
            <span>英文名: {{ row.englishName || '无' }}</span>
          </div>
          <div class="people-status">
            <el-icon><Key /></el-icon>
            <span>序号: {{ row.authorId }}</span>
          </div>
        </div>

        <div class="people-tags">
          <span class="role-tag">Author</span>
        </div>
    ...
    <el-drawer v-model="drawerVisible" :title="isEdit ? '编辑作者' : '新增作者'" size="400px">
      <el-form :model="formData" label-position="top">
        <el-form-item label="中文名">
          <el-input v-model="formData.chineseName" placeholder="请输入作者中文名" />
        </el-form-item>
        <el-form-item label="英文名">
          <el-input v-model="formData.englishName" placeholder="请输入作者英文名" />
        </el-form-item>
      </el-form>
      <template #footer>
        <div style="flex: auto">
...
const formData = reactive({ chineseName: '', englishName: '' })
...
const openDrawer = (row) => {
  if (row) {
    isEdit.value = true
    editId.value = row.authorId
    formData.chineseName = row.chineseName || ''
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      await authorApi.update(editId.value, { ...formData })
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确定要删除该作者吗？', '提示', { type: 'warning' })
    .then(async () => {
      await authorApi.delete(row.authorId)
